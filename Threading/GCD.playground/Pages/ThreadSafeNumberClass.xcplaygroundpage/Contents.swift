...
class Number {
    ...
    func changeNumber(_ value: Int, name: String) {
        print("From WorkerQueue--\(Thread.current)")
        isolationQueue.async(flags: .barrier) {
            print("From IsolationQueue--\(Thread.current)")
...
let workerQueue = DispatchQueue(label: "ConcurrentQueue.worker", attributes: DispatchQueue.Attributes.concurrent)
